...
class AIChat {
    constructor() {
        ...
        this.updateAIName();
        this.updatePersonalityDisplay(); // Initialize personality display
        this.bindEvents();
        this.loadChatHistory();
        this.updateServiceStatus();
        this.setupConsoleLogging();
        this.setupMobileViewport();
        console.log('AIChat constructor completed');
    }
        ...
        try {
            ...
            if (response.type === 'text' && this.currentService === 'localai') {
                ...
                if (imageResult) {
                    ...
                    this.hideLoading(); // Hide typing indicator before adding text
                    this.addMessage('ai', imageResult.textContent, 'text', response.thinking);
                    
                    // Then generate and add image with simple downloading message
                    if (imageResult.imagePrompt) {
                        ...
                        this.addMessage('system', '🎨 Downloading image...', 'text');
                        
                        try {
                            const imageResponse = await this.generateImage(imageResult.imagePrompt);
                            console.log('✅ Auto image generation complete!');
                            
                            // Remove downloading message and add image with prompt info
                            const downloadingMsg = this.messagesContainer.lastElementChild;
                            if (downloadingMsg && downloadingMsg.textContent.includes('Downloading image')) {
                                downloadingMsg.remove();
                            }
                            
                            this.addMessage('ai', imageResponse.content, 'image', null, imageResult.imagePrompt);
                        } catch (imageError) {
                            console.log('⚠️ Auto image generation failed:', imageError.message);
                            // Remove downloading message on error
                            const downloadingMsg = this.messagesContainer.lastElementChild;
                            if (downloadingMsg && downloadingMsg.textContent.includes('Downloading image')) {
                                downloadingMsg.remove();
                            }
                ...
                if (this.currentService === 'automatic1111' && response.type === 'image') {
                    // Direct image generation - hide typing, show downloading, then image
                    this.hideLoading();
                    this.addMessage('system', '🎨 Downloading image...', 'text');
                    ...
                    // Remove downloading message and add image with prompt
                    const downloadingMsg = this.messagesContainer.lastElementChild;
                    if (downloadingMsg && downloadingMsg.textContent.includes('Downloading image')) {
                        downloadingMsg.remove();
                    }
                    
                    this.addMessage('ai', response.content, 'image', null, message); // Use original message as prompt
                } else {
            ...
            // Don't show A1111/image generation errors to users
            if (this.currentService === 'automatic1111' || 
                error.message.includes('A1111') || 
                error.message.includes('container') ||
                error.message.includes('Failed to start A1111') ||
                error.message.includes('Image generation failed')) {
                
                console.error('🎨 Image generation error (hidden from user):', error.message);
                // Remove any downloading message that might be showing
                const downloadingMsg = this.messagesContainer.lastElementChild;
                if (downloadingMsg && downloadingMsg.textContent.includes('Downloading image')) {
                    downloadingMsg.remove();
                }
                return; // Don't show error to user for image generation
            }
            
            let errorMessage = error.message;
            let debugInfo = '';
            
            // Provide more specific error messages for non-image services
            ...
            // Show comprehensive error in UI (only for non-image services)
...
CRITICAL: Always include [IMAGE_PROMPT: ...] when describing anything visual!`;
    ...
    async generateImage(prompt, retryCount = 0) {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                // Try to parse as JSON to check for configuration errors
                try {
                    errorResponse = JSON.parse(errorText);
                    if (errorResponse.isConfigurationError) {
                        // SSH/Configuration error - throw silently without restart attempts
                        throw new Error('Image generation service unavailable (configuration issue)');
                    }
                } catch (parseError) {
                ...
                if (needsRestart) {
                    ...
                    if (retryCount >= 2) {
                        ...
                        // Don't show system message to user - just throw error silently
                    ...
                    try {
                        // Don't show restart messages to user - handle silently
                        console.log('🔄 A1111 container issue detected - automatically restarting and retrying your image generation...');
                        ...
                        if (dockerResponse.ok) {
                            ...
                            if (dockerResult.success) {
                                ...
                                // Don't show success message - handle silently
                                console.log('✅ A1111 restarted successfully - generating your image now...');
                                ...
                                try {
                                    ...
                                    // Don't show error message - just throw silently
    ...
    addMessage(sender, content, type = 'text', thinking = '', imagePrompt = '') {
        const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        const id = Date.now() + Math.random(); // Simple unique ID
        const message = { id, sender, content, type, timestamp, thinking, imagePrompt };
    ...
    renderMessage(message) {
        ...
        if (message.type === 'image') {
            let promptHTML = '';
            if (message.imagePrompt && message.imagePrompt.trim()) {
                promptHTML = `
            ...
            contentHTML = `
                <div class="message-bubble">
                    ${promptHTML}
                    <img src="${message.content}" alt="Generated image" class="message-image clickable-image" onclick="this.classList.toggle('fullscreen')" />
    ...
    startRealisticTyping() {
        ...
        // For image generation, don't show complex typing - just a simple indicator
        if (this.currentService === 'automatic1111' || this.currentService === 'comfyui') {
        ...
        // For text chat, use minimal typing pattern
        const typingDiv = document.createElement('div');
        typingDiv.className = 'message ai';
        typingDiv.id = 'typing-indicator';
        typingDiv.innerHTML = `
        ...
        // Start minimal typing pattern - much less activity
        this.simulateMinimalTyping();
    }

    simulateMinimalTyping() {
        const typingIndicator = document.getElementById('typing-indicator');
        if (!typingIndicator) return;

        // Much simpler pattern: show for 2 seconds, then hide for 10 seconds
        let isActive = true;
    ...
    hideLoading() {
        // Remove all typing indicators (in case there are duplicates)
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM loaded, initializing AIChat');
    try {
        new AIChat();
